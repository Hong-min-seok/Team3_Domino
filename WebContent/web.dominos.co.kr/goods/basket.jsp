...
<body>
	...
	<div id="wrap">
		...
		<div id="container">
    		...
    		<div align="center">
    			<h1 class="page-title" style="font-size: 36px; width: 80%; text-align: left; padding: 50px">장바구니</h1>
    			<hr/ style="width: 80%; border-left-width: thick;">
					<form action="./OderOk.pro" method="post">
						<table border=1 style="border-collapse: collapse;">
							<% MemberBean memberbean = (MemberBean)request.getAttribute("memberDetail"); %>
									<tr>
					    				<th>
					    					주 문 자
					    				</th>
					    				<td>
					    					...
					    					<input type="hidden" name="member_code" value=<%=memberbean.getMember_code() %>>
					    				</td>
									</tr>
									<tr>
					    				<th>
					    				...
					    				<td>
					    				<%
					    					String str = memberbean.getMember_phone();
									...
									<tr>
					    				...
					    				<td>
					    					<%=memberbean.getMember_phone() %>
				<%--                     		<input type="text" name="tel1" id="tel1" maxlength="4" title="휴대전화번호" value=<%=phone[0] %>> - --%>
				<%--                     		<input type="text" name="tel2" id="tel2" maxlength="4" title="휴대전화번호" value=<%=phone[1] %>> - --%>
				<%--                     		<input type="text" name="tel3" id="tel3" maxlength="4" title="휴대전화번호" value=<%=phone[2] %>> --%>
					    				</td>
									</tr>
						</table>
						<br>
						<div style="width: 80%" align="left">
				          <select name="store_code" style="width: 200px">
						 <%
				               StoreDAO storedao = new StoreDAO();
				               List storelist = storedao.getStoreList();
				         
				               for(int i = 0; i < storelist.size(); i++){
				               StoreBean bean = (StoreBean)storelist.get(i);
				          %>
				                  <option value=<%=bean.getStore_code() %>><%=bean.getStore_name() %></option>
				          <%}%> 
					...
					<br><br>
						<table style="width: 80%" cellpadding="0" cellspacing="0">
							...
							<tr style="border-bottom: 1px solid gray">
								<%
								List pizzaList = (ArrayList) request.getAttribute("pizzaList");
								...
								for (int i = 0; i < pizzaList.size(); i++) {
									...
									ProductBean pizzabean = (ProductBean) pizzaList.get(i);
				
									int pcode = pizzabean.getPizza_code();
									String pcodeS = Integer.toString(pcode);
									String pname = pizzabean.getPizza_name();
									String ptemp = "pizzacode" + pcode;
									String pimage = pizzabean.getPizza_image();
									
									if(session.getAttribute("p" + ptemp)!=null) {
								%>
								<th><img src="../../pizza_image/<%=session.getAttribute("p" + pimage)%>" alt="<%=session.getAttribute("p" + pname)%>" style="width: 150px; height: 150px; padding: 5px"/></th>
								<th><%=session.getAttribute("p" + pname)%><br> <small><%=session.getAttribute(pcodeS + "dough")%> / <%=session.getAttribute(pcodeS + "size")%></small>
									<input type="hidden" name="pizza_code" value=<%=session.getAttribute("p" + pcode)%>>
									<input type="hidden" name="pizza_size" value=<%=session.getAttribute(pcodeS + "size")%>></th>
				<%-- 					<input type="hidden" name="dough_code" value=<%=session.getAttribute(pcodeS + "dough")%>> --%>
								<th>
								<%
									for (int j = 0; j < toppingList.size(); j++) {
										
										ProductBean toppingbean = (ProductBean) toppingList.get(j);
									
										int tcode = toppingbean.getTopping_code();
										String tcodeS = Integer.toString(tcode);
										String tname = toppingbean.getTopping_name();
										String ttemp = "toppingcode" + tcode;
										
										if(session.getAttribute("t" + ttemp)!=null) {
								%> 
										<%=session.getAttribute("t" + tname)%>/<%=session.getAttribute("t" + ttemp)%><br>
										<input type="hidden" name="add<%=j+1 %>" value=<%=session.getAttribute("t" + tname)%>>
								<%
										}
									}
								%>
								</th>
								<th><%=session.getAttribute("p" + ptemp)%></th>
									<input type="hidden" name="pizzaCnt" value=<%=session.getAttribute("p" + ptemp)%>>
							</tr>
							<%
									}
								}
							...
							for(int i=0; i < sideList.size(); i++){
								...
								int scode = bean.getSide_code();
								String scodeS = Integer.toString(scode);
								String sname = bean.getSide_name();
								String stemp = "sidecode"+scode;
								String simage = bean.getSide_image();
								
								if(session.getAttribute("s" + stemp)!=null) {
							%>
								<th><img src="../../side_image/<%=session.getAttribute("s"+simage)%>" alt="<%=session.getAttribute("s"+sname)%>" style="width: 150px; height: 150px; padding: 5px"/></th>
								<th><%=session.getAttribute("s"+sname)%></th>
								<input type="hidden" name="side_code" value=<%=session.getAttribute("s" + scode)%>>
								<th></th>
								<th><%=session.getAttribute("s"+stemp)%></th>
							</tr>
							<%
								}
							}
							...
							for(int i=0; i < otherList.size(); i++){
								...
								int ocode = bean.getOther_code();
								String ocodeS = Integer.toString(ocode);
								String oname = bean.getOther_name();
								String otemp = "othercode"+ocode;
								String oimage = bean.getOther_image();
								
								if(session.getAttribute("o" + otemp)!=null) {
							%>
								<th><img src="../../other_image/<%=session.getAttribute("o"+oimage)%>" alt="<%=session.getAttribute("o"+oname)%>" style="width: 150px; height: 150px; padding: 5px"/></th>
								<th><%=session.getAttribute("o"+oname)%></th>
									<input type="hidden" name="side_code" value=<%=session.getAttribute("o" + oname)%>>
								<th></th>
								<th><%=session.getAttribute("o"+otemp)%></th>
							</tr >
							<%
								}
							}
						...
						<br> <input type="submit" value="결제하기" class="btn-type v6" style="margin-bottom: 100px; vertical-align: middle; color: white;">
					</form>
				</div>
    		</section>
    	</div>
