	...
	<!-- ------------------------------------------------------------------------------------------------- -->
		...
		<div id="container">
			<section id="content">
				...
				<h1 class="page-title" style="font-size: 36px; width: 80%; text-align: left; padding: 50px">월간 판매량</h1>
    			<hr/ style="width: 80%; border-left-width: thick;">
    			
				<div id="statistic" style="width:80%; margin-top: 50px; margin-bottom: 100px; text-align: left;">
					
					<div class="title-wrap-center" style="margin-bottom: 20px">
						<h3 class="title-type"><b>피자랭킹 Top3</b></h3>
					</div>
					<div class="menu-list" align="center">

								<%
								List ranklist = (List)request.getAttribute("rank");
								%>
								<ul>
									<!-- 피자&사이드 -->
									<%
										for (int i = 0; i < ranklist.size(); i++) {
											ProductBean rbean = (ProductBean)ranklist.get(i);
									%>
									<li>
										<div class="prd-img">
												<img class="lazyload"
												src="../../pizza_image/<%=rbean.getPizza_image() %>"
												alt="<%=rbean.getPizza_image() %>" />
										</div>
										<div class="prd-cont">
											<div class="subject">
												<%=rbean.getPizza_name() %>
												<div class="label-box">
													<span class="label sale"><%=i+1 %>위</span>
												</div>
											</div>
										</div>
					...
					<%
						List statisticlist = (ArrayList)request.getAttribute("statisticlist");
						int month = 0;
						//System.out.println(statisticlist.toString());
					%>
					<%
					for(int i=0; i<statisticlist.size(); i++) {
							StatisticBean bean = (StatisticBean)statisticlist.get(i);
							
							if(month != bean.getMonth()){ 
								month = bean.getMonth(); %>
								<br>
								<div style="font-size: 24pt"><%=bean.getMonth() %>월 판매량</div>
							<%} %>
								<div style="font-size: 18pt;">&nbsp;&nbsp;&nbsp;&nbsp;[<%=bean.getPizza_code() %>] <%=bean.getPizza_name() %> : <%=bean.getCount() %>개</div>
