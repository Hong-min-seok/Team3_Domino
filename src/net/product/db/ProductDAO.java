...
public class ProductDAO {
	...
	// 도우 상세
	public ProductBean getDoughDetail(int code) throws Exception {
		ProductBean bean = null;
		try {
			pstmt = conn.prepareStatement("select * from dough where dough_code = ?");
			pstmt.setInt(1, code);
			...
			if (rs.next()) {
				bean = new ProductBean();
				bean.setDough_code(rs.getInt("dough_code"));
				bean.setDough_image(rs.getString("dough_image"));
				bean.setDough_name(rs.getString("dough_name"));
				bean.setDough_price(rs.getInt("dough_price"));
			}
			return bean;
		} catch (Exception ex) {
			System.out.println("getDoughDetail 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	// 사이드 상세
	public ProductBean getSideDetail(int code) throws Exception {
		ProductBean bean = null;
		try {
			pstmt = conn.prepareStatement("select * from side where side_code = ?");
			pstmt.setInt(1, code);
			...
			if (rs.next()) {
				bean = new ProductBean();
				bean.setSide_code(rs.getInt("side_code"));
				bean.setSide_image(rs.getString("side_image"));
				bean.setSide_name(rs.getString("side_name"));
				bean.setSide_price(rs.getInt("side_price"));
			}
			return bean;
		} catch (Exception ex) {
			System.out.println("getSideDetail 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	// 토핑 상세
	public ProductBean getToppingDetail(int code) throws Exception {
		ProductBean bean = null;
		try {
			pstmt = conn.prepareStatement("select * from topping where topping_code = ?");
			pstmt.setInt(1, code);
			...
			if (rs.next()) {
				bean = new ProductBean();
				bean.setTopping_code(rs.getInt("topping_code"));
				bean.setTopping_image(rs.getString("topping_image"));
				bean.setTopping_name(rs.getString("topping_name"));
				bean.setTopping_price(rs.getInt("topping_price"));
			}
			return bean;
		} catch (Exception ex) {
			System.out.println("getToppingDetail 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	// 음료/기타 상세
	public ProductBean getOtherDetail(int code) throws Exception {
		ProductBean bean = null;
		try {
			pstmt = conn.prepareStatement("select * from other where other_code = ?");
			pstmt.setInt(1, code);
			...
			if (rs.next()) {
				bean = new ProductBean();
				bean.setOther_code(rs.getInt("other_code"));
				bean.setOther_image(rs.getString("other_image"));
				bean.setOther_name(rs.getString("other_name"));
				bean.setOther_price(rs.getInt("other_price"));
			}
			return bean;
		} catch (Exception ex) {
			System.out.println("getOtherDetail 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	// 장바구니 목록 보기 getBasketList
	public List getBasketList() {

		String sql = "SELECT * FROM PIZZA RIGHT JOIN SIDE ON (PIZZA.PIZZA_CODE = SIDE.SIDE_CODE) "+
				"FULL JOIN TOPPING ON (SIDE.SIDE_CODE = TOPPING.TOPPING_CODE) "+
				"LEFT JOIN OTHER ON (TOPPING.TOPPING_CODE = OTHER.OTHER_CODE) "+
				"LEFT JOIN DOUGH ON (OTHER.OTHER_CODE = DOUGH.DOUGH_CODE)";
	...
	// 주문 하기 setOder
	public boolean setOder(ProductBean product, String pizzaCnt) {

		int result = 0;

		String sql = "INSERT INTO user_order values(?,?,TO_DATE(SYSDATE,'YYYY-MM-DD HH24:MI:SS'),?,?,?,?,?,?,?,?,?,?,?)";

		try {

			// setOder
				System.out.println("주문 하러옴 dao");
				int num = Integer.parseInt(pizzaCnt);

				for(int i=0; i<num; i++) {
					pstmt = conn.prepareStatement(sql);
					
					if(product.getMember_code()!=0)
						pstmt.setInt(1,product.getMember_code());
					else
						pstmt.setNull(1, num);
					
					if(product.getNonmember_code()!=0)
						pstmt.setInt(2,product.getNonmember_code());
					else
						pstmt.setNull(2, num);
					
					if(product.getStore_code()!=0)
						pstmt.setInt(3,product.getStore_code());
					else
						pstmt.setNull(3, num);
					
					if(product.getPizza_code()!=0)
						pstmt.setInt(4,product.getPizza_code());
					else
						pstmt.setNull(4, num);
					
					
					pstmt.setString(5,product.getPizza_size());
					
					if(product.getDough_code()!=0)
						pstmt.setInt(6,product.getDough_code());
					else
						pstmt.setNull(6, num);
					
					if(product.getSide_code()!=0)
						pstmt.setInt(7,product.getSide_code());
					else
						pstmt.setNull(7, num);
					
					if(product.getOther_code()!=0)
						pstmt.setInt(8,product.getOther_code());
					else
						pstmt.setNull(8, num);
					
					if(product.getAdd1()!=0)
						pstmt.setInt(9,product.getAdd1());
					else
						pstmt.setNull(9, num);
					
					if(product.getAdd2()!=0)
						pstmt.setInt(10,product.getAdd2());
					else
						pstmt.setNull(10, num);
					
					if(product.getAdd3()!=0)
						pstmt.setInt(11,product.getAdd3());
					else
						pstmt.setNull(11, num);
					
					if(product.getAdd4()!=0)
						pstmt.setInt(12,product.getAdd4());
					else
						pstmt.setNull(12, num);
					
					if(product.getAdd5()!=0)
						pstmt.setInt(10,product.getAdd5());
					else
						pstmt.setNull(13, num);
					...
					System.out.println("주문 성공dao");
					return true;
				}
		}catch(Exception e){
			System.out.println("setOder 에러 : "+e);
		}finally{
			if(rs!=null) try{rs.close();}catch(SQLException e){}
			if(pstmt!=null) try{pstmt.close();}catch(SQLException e){}
		}
