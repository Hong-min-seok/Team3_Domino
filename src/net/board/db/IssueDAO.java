...
public class IssueDAO {
	...
	public List getIssueList(){
		System.out.println("getIssueList함수 들어옴");
		...
		try{
			...
			System.out.println("getIssueList 에러 : " + ex);
		}finally{
			if(rs!=null) try{rs.close();}catch(SQLException ex){}
			if(pstmt!=null) try{pstmt.close();}catch(SQLException ex){}
		}
	...
	public boolean issueWrite(IssueBean bean) throws Exception {
		...
		System.out.println("issueWrite 함수 들어옴");
		...
		try {
			...
			if (result == 0)
				return false;
			System.out.println("여기는 IssueDAO의 issueWrite : 등록 성공!");
			return true;

		} catch (Exception ex) {
			System.out.println("issueWrite 에러 : " + ex);
		} finally {
			if (rs != null)
				try {
					rs.close();
	...
	public IssueBean getIssueDetail(int num) throws Exception{
		...
		System.out.println("getIssueDetail 함수 실행");
		IssueBean bean = null;
		try{
			...
			System.out.println("getIssueDetail 에러 : " + ex);
		}finally{
			if(rs!=null)try{rs.close();}catch(SQLException ex){}
			if(pstmt !=null)try{pstmt.close();}catch(SQLException ex){}
		}
	...
	public boolean issueDelete(int num){
		...
		try{
			...
			System.out.println("issueDelete 에러 : "+ex);
		}finally{
			try{
